...
struct thread_pool
{
	...
	pthread_mutex_t ready_tasks_mutex;
...
thread_pool_new(int max_thread_count, struct thread_pool **pool)
{
	...
	*pool = calloc(1, sizeof(struct thread_pool));
	...
	pthread_mutex_init(&(*pool)->ready_tasks_mutex, NULL);
...
thread_pool_delete(struct thread_pool *pool)
{
	...
	pthread_mutex_destroy(&pool->ready_tasks_mutex);
...
thread_task_join(struct thread_task *task, void **result)
{
	...
	pthread_mutex_lock(&task->pool->ready_tasks_mutex);
	while (task->status != TPOOL_TASK_FINISHED) {
		pthread_cond_wait(&task->pool->ready_tasks_cv, &task->pool->ready_tasks_mutex);
	}

	task->is_joined = true;
	pthread_mutex_unlock(&task->pool->ready_tasks_mutex);
...
thread_task_timed_join(struct thread_task *task, double timeout, void **result)
{
	...
	pthread_mutex_lock(&task->pool->ready_tasks_mutex);
	while (task->status != TPOOL_TASK_FINISHED) {
		pthread_cond_timedwait(&task->pool->ready_tasks_cv, &task->pool->ready_tasks_mutex, &timeout_time);
