...
chat_client_new(const char *name)
{
	...
	strcpy(client->buffers.next->data, name);
...
chat_client_delete(struct chat_client *client)
{
	...
	for (size_t i = 0; i < client->authors_size; i++) {
...
chat_client_connect(struct chat_client *client, const char *addr)
{
	...
	if (getaddrinfo(addr, NULL, NULL, &address) != 0) {
		close(client->socket);
...
receive_from_server(struct chat_client *client)
{
	while (true) {
		...
		while (size > 0) {
			size -= parse_data(client, buffer, size);
		}
	}
}
...
parse_data(struct chat_client *client, const char *buffer, ssize_t size)
{
	if (client->messages.next == NULL || client->last_message->status == MESSAGE_STATUS_READY) {
		client->messages.next = calloc(1, sizeof(*client->messages.next));
		client->messages.next->status = MESSAGE_STATUS_READING_AUTHOR;
		client->last_message = client->messages.next;
	}
...
get_author(char *author, ssize_t author_size, struct chat_client *client)
{
	...
	if (client->authors_size == client->authors_capacity) {
		...
		client->authors = realloc(client->authors, client->authors_capacity);
	}
